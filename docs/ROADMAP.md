...
| 4 | Data quality layer + `DATA_QUALITY.md` | ⬜ |
| 5 | Candle engine (1s…15m from ticks; forming candles marked) | ⬜ |
...
\* Phase 30 (per build directive) gates everything after Phase 3: the live
validation must pass before the candle/feature/ML work starts.

## Immediate next step

The owner puts `PO_SSID` into `.env` (instructions in `.env.example`), then:
